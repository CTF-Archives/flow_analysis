...
# import utils from dpkt
from utils.core_dpkt_old import *


if __name__ == "__main__":

    capture_file = "./examples/wdb2024-misc-03.pcapng"
    ...
    for session_id, req, res in traffic_data_matched_HTTPSessions:
        request_path = req.decode(errors="ignore").replace("\r\n", "\n").split("\n")[0].split(" ")[1]
        request_context = req.decode(errors="ignore").replace("\r\n", "\n").split("\n\n")[1]
        response_status = res.decode(errors="ignore").replace("\r\n", "\n").split("\n")[0].split(" ")[1]
